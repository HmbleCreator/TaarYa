...
class VectorSearch:
    ...
    def ensure_collection(
        self, name: Optional[str] = None, vector_size: int = VECTOR_SIZE
    ) -> None:
        """
    ...
    def search_similar(
        ...
        try:
            ...
            if collection not in [c.name for c in collections]:
                logger.warning(
                    f"Collection '{collection}' not found — no documents indexed yet"
                )
                return []
    ...
    def search_by_vector(
        self, vector: List[float], collection: Optional[str] = None, limit: int = 10
    ) -> List[Dict[str, Any]]:
        """
    ...
    def get_collection_info(self, collection: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            points_count = getattr(info, "points_count", None) or getattr(
                info, "points_count", 0
            )
            return {
                "name": collection,
            ...
            return {"name": collection, "exists": False, "error": str(e)}
